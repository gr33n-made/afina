...
std::string ParseRunner::Run() {
    ...
    while (total_parsed != len_recv || is_create_command) {
        ...
        if (len_args) {
            ...
            if (ParseArgs(recv + total_parsed, len_args - parsed_args, args + parsed_args, parsed_args)) {
                total_parsed += parsed_args - prev_parsed_args;
        ...
        ////////////////////////////////////////////////////////////////
        size_t parsed = 0;
        if (parser.Parse(recv + total_parsed, len_recv - total_parsed, parsed)) {
            total_parsed += parsed;
