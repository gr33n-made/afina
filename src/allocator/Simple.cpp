...
Simple::Simple(void *base, size_t size) : _base(base), _base_len(size) {
    ...
    //new free chunk
    FreeChunk *free_chunk = (FreeChunk*)_base; //_tail->b_free_chunk;
...
Pointer Simple::alloc(size_t N) {
    ...
    while(curr_f_chunk != nullptr) { //trying to find chunk of suitable size
...
void Simple::realloc(Pointer &p, size_t N) {
    ...
    if (chunk->size >= N) {
        if (chunk->size - N <= sizeof(FreeChunk)) //if we have enough space
            return;

        size_t old_chunk_size = chunk->size;
        chunk->size = N;
        create_f_chunk((FreeChunk*)((char*)chunk + sizeof(Chunk) + N), old_chunk_size - (sizeof(Chunk) + N));
        return;
    }
...
void Simple::defrag() {
    ...
    while(curr_chunk != (Chunk*)_tail->b_free_chunk) {
        ...
        curr_chunk=(Chunk*)((char*)curr_chunk + sizeof(Chunk) + curr_chunk->size);
    }

    Desc *desc_old_chunk = _tail->b_free_chunk->desc;
    _tail->b_free_chunk = (FreeChunk*)((char*)_tail->b_free_chunk - size_move);
    _tail->b_free_chunk->desc = desc_old_chunk;
...
Simple::FreeChunk* Simple::join_chunks(FreeChunk* free_chunk) {
    /*if(free_chunk == _tail->b_free_chunk) //if it's last one
        return nullptr;*/
